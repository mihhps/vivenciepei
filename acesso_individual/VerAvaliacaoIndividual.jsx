...
// Configurações e Funções Helper (mantidas)
const NIVEL_CONFIG = {
  NR: {
    cor: "#f8d0d9",
...
const buscarHistoricoCompletoDoAluno = async (alunoId, dbInstance) => {
  // dbInstance do useAuth
  ...
  for (const colecaoNome of colecoesAvaliacao) {
    // A QUERY É POR alunoId. O filtro de segurança deve estar em quem acessa o componente.
    const q = query(
      collection(dbInstance, colecaoNome),
...
// Componente DetalhesContent (MANTIDO)
...
// Componente ComparacaoContent (MANTIDO)
        ...
        <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4">
          ...
          <div className="w-full sm:w-1/2">
            ...
            <div className="relative">
              ...
              >
                {historicoAvaliacoes
                  .filter((hist) => hist.id !== avaliacaoComparacaoId)
                  .slice()
                  .reverse()
                  .map((hist) => (
          ...
          {/* SELETOR PARA AVALIAÇÃO DE COMPARAÇÃO (B) */}
          <div className="w-full sm:w-1/2">
            <p className="text-sm font-semibold text-gray-600 mb-1 text-center">
            ...
            <div className="relative">
              ...
              >
                {historicoAvaliacoes
                  .filter((hist) => hist.id !== avaliacaoBase.id)
                  .slice()
                  .reverse()
                  .map((hist) => (
                    <option key={hist.id} value={hist.id}>
...
function VerAvaliacaoIndividual() {
  // RENOMEADO
  const { id } = useParams();
  // Assume que o useAuth fornece a instância do Firestore (dbInstance)
  // e os dados do usuário (user).
  ...
  useEffect(() => {
    ...
    const carregarAvaliacao = async () => {
      ...
      try {
        // Usa a função de busca com o dbInstance (que garante o acesso)
        ...
        if (historico.length > 0) {
          ...
          if (historico.length >= 2) {
            // Seleciona a segunda mais recente para comparação automática
          ...
          setError(`Nenhuma avaliação válida encontrada para o aluno.`);
        }
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans p-4 sm:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Seletor de Avaliação Principal (MANTIDO) */}
...
export default VerAvaliacaoIndividual; // EXPORT RENOMEADO
